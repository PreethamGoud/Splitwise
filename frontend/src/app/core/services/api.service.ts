...
export class ApiService {
  ...
  healthCheck(): Observable<string> {
    return this.http.get(`${this.base}/public/healthCheck`, {
      responseType: 'text',
    });
  }
  ...
  getBalanceAll(): Observable<Record<string, number> | UserDtoOrError> {
    return this.http.get<Record<string, number> | UserDtoOrError>(
      `${this.base}/user/getBalance/all`,
    );
  }

  getSettledBalanceAll(): Observable<SettleUpDto[] | UserDtoOrError> {
    return this.http.get<SettleUpDto[] | UserDtoOrError>(
      `${this.base}/user/getBalance/all/settled`,
    );
  }
  ...
  addRoleToUser(
    userName: string,
    roleName: string,
  ): Observable<boolean | UserDtoOrError> {
  ...
  getGroupMembers(
    groupName: string,
  ): Observable<{ userId: number; userName: string }[] | UserDtoOrError> {
    return this.http.get<
      { userId: number; userName: string }[] | UserDtoOrError
  ...
  createGroup(body: CreateGroupRequest): Observable<GroupDto | UserDtoOrError> {
    return this.http.post<GroupDto | UserDtoOrError>(
      `${this.base}/group`,
      body,
    );
  }
  ...
  /**
   * Backend returns plain text (e.g. "User added successfully!") rather than JSON,
   * so request must use `responseType: 'text'` to avoid a parse error.
   */
  addUserToGroup(groupName: string, userName: string): Observable<string> {
  ...
  createExpense(body: CreateExpenseDto): Observable<string> {
    return this.http.post(`${this.base}/expense`, body, {
      responseType: 'text',
    });
  }
  ...
  getSettleUpGroup(
    groupName: string,
  ): Observable<SettleUpDto[] | UserDtoOrError> {
    return this.http.get<SettleUpDto[] | UserDtoOrError>(
  ...
  getAllSettledUserGroup(
    groupName: string,
  ): Observable<AllGroupsSettledDto | UserDtoOrError> {
    return this.http.get<AllGroupsSettledDto | UserDtoOrError>(
